            ...
            <div className="bill-view-container">
                ...
                <div style={{ width: '100%', display: 'flex', justifyContent: 'center', overflow: 'visible' }}>
                    <div className="bill-scale-wrapper" style={{ 
                        ...
                        {chunks.map((chunkItems, pageIdx) => {
                            const pagePcs = chunkItems.reduce((acc, item) => acc + (item.pcs || 0), 0);
                            const pageMeters = chunkItems.reduce((acc, item) => acc + (item.meters || 0), 0);
                            const pageSubTotal = chunkItems.reduce((acc, item) => acc + (item.amount || 0), 0);

                            const showPageBreak = pageIdx < chunks.length - 1 || !!bill.ewayBillDetails?.uniqueNo;

                            return (
                                <div 
                                    className="sagar-bill-paper" 
                                    id={pageIdx === 0 ? "printable-bill" : undefined} 
                                    style={{ pageBreakAfter: showPageBreak ? 'always' : 'auto', marginBottom: '40px' }} 
                                    key={pageIdx}
                                >
                                        ...
                                        <table style={{ tableLayout: 'fixed', width: '100%', borderCollapse: 'collapse', fontFamily: 'Inter, system-ui, sans-serif', fontSize: '12px' }}>
                                            ...
                                            <tbody>
                                                {chunkItems.map((item, idx) => {
                                                    ...
                                                    return (
                                                        <tr key={idx}>
                                                            <td style={{ borderRight: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0', padding: '10px 12px', textAlign: 'center', color: '#64748b' }}>{globalIdx}</td>
                                                ...
                                                {[...Array(Math.max(0, itemsPerPage - chunkItems.length))].map((_, i) => (
                                                    <tr key={`empty-${i}`}>
                                            ...
                                            <tfoot>
                                                <tr style={{ background: '#f1f5f9' }}>
                                                    ...
                                                    <td style={{ borderTop: '2px solid #1e3a8a', borderLeft: '1px solid #e2e8f0', padding: '12px', textAlign: 'right', fontWeight: '800', color: '#1e3a8a', fontSize: '13px' }}>{pagePcs.toFixed(2)}</td>
                                                    <td style={{ borderTop: '2px solid #1e3a8a', borderLeft: '1px solid #e2e8f0', padding: '12px', textAlign: 'right', fontWeight: '800', color: '#1e3a8a', fontSize: '13px' }}>{pageMeters.toFixed(2)}</td>
                                                    <td style={{ borderTop: '2px solid #1e3a8a', borderLeft: '1px solid #e2e8f0', padding: '12px' }}></td>
                                                    <td style={{ borderTop: '2px solid #1e3a8a', borderLeft: '1px solid #e2e8f0', padding: '12px', textAlign: 'right', fontWeight: '900', color: '#1e3a8a', fontSize: '15px' }}>Rs.{pageSubTotal.toFixed(2)}</td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
